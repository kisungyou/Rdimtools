...
do.kudp <- function(X, ndim=2, type=c("proportion",0.1),
  ...
  if (!check_NumMM(bandwidth,0,1e+10,compact=TRUE)){stop("* do.kudp : 'bandwidth' should be a nonnegative real number.")}
  ...
  #   6. kernelized
  K = exp(-(as.matrix(dist(pX))^2)/bandwidth); diag(K) = 0; ## this part i'm not sure
  H = K*(nbdstruct$mask)
  D = diag(colSums(H))
